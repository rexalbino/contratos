    ...
    <body class="#212121 grey darken-4">
        <?php
            ...
            if(empty($nombre) || empty($inicio) || empty($fin) || empty($area) || empty($tipocontrato) || empty($thirdp)){
                ...
                $sql2="UPDATE `contratos` SET `num_contrato` = '$nombre', `date_start` = '$inicio', `date_end` = '$fin', `area` = '$area', `tipo_contrato` = '$tipocontrato', `third_party` = '$thirdp', `contrato` = '$archivod' WHERE `contratos`.`id_contrato` = $id;";
                mysqli_query($link,$sql2);
                echo "<script language=javaScript>alert('Articulo cargado exitosamente');</script>";
                
            }
        
        echo "<h1 class='center-align #afb42b lime darken-2'>Actualizando contrato</h1>";
        ?>
        </br>
        </br>
        </br>
